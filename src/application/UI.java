...
public class UI {
	...
	public static PosicaoXadrez readPosicaoXadrez(Scanner sc) {
		...
		return new PosicaoXadrez(coluna, linha);
			...
			throw new InputMismatchException("Error Lendo posição do Xadrez, valores validos de a1 a h8.");
		}
	}
	
	public static void printPartida(PartidaDeXadrez partidaDeXadrez, List<PeçadeXadrez> capturada) {
	...
	private static void printPeçasCapturadas(List<PeçadeXadrez> capturada) {
		List<PeçadeXadrez> Branca = capturada.stream().filter(x -> x.getCor() == Cor.BRANCA).collect(Collectors.toList());
		List<PeçadeXadrez> Preta = capturada.stream().filter(x -> x.getCor() == Cor.PRETA).collect(Collectors.toList());
		System.out.println("PeçasCapturadas");
